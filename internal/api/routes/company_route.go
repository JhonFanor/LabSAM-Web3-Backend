...
func (cr *CompanyRoutes) Routes() {
	company := cr.Router.Group("/api/company")
	{
		...
		company.GET("/count-by-subtopic", cr.CompanyController.CountComapnyBySubtopic)
		...
		company.DELETE("/:id", cr.TokenMiddleware.ValidateToken(), cr.CompanyController.DeleteCompany)
	}
}
